...
#if LARGE_BINARY < 2
// don't include the big lookup table, generate it instead
CRC32KTable::CRC32KTable() {
      ...
      for (unsigned int slice = 1; slice < MaxSlice; slice++)
       for (int i = 0; i <= 0xFF; i++)
        tab[slice][i] = (tab[slice - 1][i] >> 8) ^ tab[0][tab[slice - 1][i] & 0xFF];
    }
#else
CRC32KTable::CRC32KTable() :
    tab
