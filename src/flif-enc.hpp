...
bool flif_encode(IO& io, Images &images, std::vector<std::string> transDesc =
                 ...
                 int lookback = 1, int divisor=CONTEXT_TREE_COUNT_DIV, int min_size=CONTEXT_TREE_MIN_SUBTREE_SIZE, int split_threshold=CONTEXT_TREE_SPLIT_THRESHOLD, int cutoff=2, int alpha=19, int crc_check=-1, int loss=0, int predictor=0);
